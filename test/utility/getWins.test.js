...
describe('getWins (integration)', () => {
	...
	test('should return empty array for player with no guesses', () => {
		const wins = getWins('August', TEST_DB_PATH);
		expect(wins).toEqual([]);
		expect(Array.isArray(wins)).toBe(true);
	});

	test('should return wins for August', () => {
	...
	test('should return only wins, not losses', () => {
		...
		makeGuess('August', 'Another Winner', '3', '3', TEST_DB_PATH);
	...
	test('should not include pending guesses (null result)', () => {
		makeGuess('Grace', 'Winner', '1', '1', TEST_DB_PATH);
		writeResult('Grace', 'win', 'Sword', 1, TEST_DB_PATH);

		makeGuess('Grace', 'Pending', '2', '2', TEST_DB_PATH);
	...
	test('should return multiple wins in correct order', () => {
		makeGuess('August', 'First Win', '1', '1', TEST_DB_PATH);
		writeResult('August', 'win', 'Sword', 1, TEST_DB_PATH);

		makeGuess('August', 'Second Win', '2', '2', TEST_DB_PATH);
		writeResult('August', 'win', 'Axe', 1, TEST_DB_PATH);

		makeGuess('August', 'Third Win', '3', '3', TEST_DB_PATH);
		writeResult('August', 'win', 'Spear', 1, TEST_DB_PATH);
	...
	test('should work with pre-populated test data', () => {
		createTestDataWithGuesses();

		const augustWins = getWins('August', TEST_DB_PATH);
		const graceWins = getWins('Grace', TEST_DB_PATH);
	...
	test('should return empty array when player has only losses', () => {
		makeGuess('Grace', 'First Loss', '1', '1', TEST_DB_PATH);
		writeResult('Grace', 'lose', 'Sword', 0, TEST_DB_PATH);

		makeGuess('Grace', 'Second Loss', '2', '2', TEST_DB_PATH);
	...
	test('should include all win properties', () => {
		makeGuess('August', 'Test Contestant', '7', '14', TEST_DB_PATH);
		writeResult('August', 'win', 'Test Weapon', 1, TEST_DB_PATH);
	...
	test('should track wins independently for both players', () => {
		makeGuess('August', 'August Win', '1', '1', TEST_DB_PATH);
		writeResult('August', 'win', 'Katana', 1, TEST_DB_PATH);

		makeGuess('Grace', 'Grace Win 1', '2', '2', TEST_DB_PATH);
		writeResult('Grace', 'win', 'Sword', 1, TEST_DB_PATH);
